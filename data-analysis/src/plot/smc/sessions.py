...
def auth(input_dir, output_dir, cell_size = 20, threshold = -80):
    ...
    nr_sessions = database.select(table)['session_cnt'].sum()
    ...
    fig = plt.figure(figsize = (1.0 * 3.75, 3.0))
    axs = []
    ...
    axs[-1].set_title('%s' % (plot_configs['title']))
    ...
    axs[-1].set_xlabel(plot_configs['x-label'])
    # y-axis
    axs[-1].set_ylabel(plot_configs['y-label'])
...
def esses(input_dir, output_dir, cell_size = 20, threshold = -80):
    ...
    # cdfs
    fig = plt.figure(figsize = (4.0 * 3.0, 3.0))
    axs = []
    for s, stat in enumerate(plot_configs.keys()):

        axs.append(fig.add_subplot(1, 4, s + 1))
    ...
    table = ('/esses/total/aps/%s/%s' % (cell_size, int(abs(threshold))))
    if table not in database.keys():
        sys.stderr.write("""[ERROR] database not available (%s). abort.\n""" % (db))
        return
    ...
    axs[3].set_title('%s' % (plot_configs['essid_cnt']['title']))
    plot.utils.cdf(axs[3], data, metric = 'essid_cnt', plot_configs = plot_configs['essid_cnt'])
...
#       - map of rss (?)
def signal_quality(input_dir, output_dir, cell_size = 20, threshold = -80):
