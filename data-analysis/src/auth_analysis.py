...
def extract_ap_stats(data, session_id, ap_mapping_info):
    ...
    start = time.time()
    # for each unique ap mac address, extract first and last 'sensing'
    # event
    ap_stats = []
...
def draw_on_map(ap_mapping_info, out_dir):

    gps_lats = defaultdict(list)
    gps_lons = defaultdict(list)
    ...
    for ap in ap_mapping_info:
        ...
        gps_lats[auth[0]].append(gps_lat)
        gps_lons[auth[0]].append(gps_lon)
    ...
    for auth in gps_lats:

        gmap = gmplot.GoogleMapPlotter(porto_lat, porto_lon, 14)
        gmap.scatter(gps_lats[auth], gps_lons[auth], auth_colors[auth], marker = False)
...
def analyze_sessions(file_name, out_dir, median_speed_thrshld = 5.5):
    ...
    if set(processed_files.values()).issubset(set([f for f in sorted(glob.glob(os.path.join(processed_dir, '*.csv')))])):

        for stat in processed_files:
            session_stats[stat] = pd.read_csv(processed_files[stat])

        print("auth_analysis::analyze_sessions() : read data from processed .csv files")
        ...
        session_stats['ap_locations'] = draw_on_map(ap_mapping_info, out_dir)

    # plot an histogram of the list
    fig_1 = plt.figure(figsize = (12, 6))

    ax1 = fig_1.add_subplot(121)
    ax1.xaxis.grid(False)
    ax1.yaxis.grid(True)
    ax1.hist(session_stats['median_scanning_dist']['ds'], histtype = 'bar', rwidth = 0.8)
    ax1.set_xlabel("scanning distance (m)")
    ax1.set_ylabel("nr. of sessions")

    ax2 = fig_1.add_subplot(122)
    ax2.xaxis.grid(False)
    ax2.yaxis.grid(True)
    ax2.hist(session_stats['median_speeds']['speed'], histtype = 'bar', rwidth = 0.8)
    ax2.set_xlabel("median speeds (m/s)")
    ax2.set_ylabel("nr. of sessions")

    fig_1.tight_layout()
    fig_1.subplots_adjust(top = 0.95)
    plt.savefig(os.path.join(out_dir, "sessions-histograms.pdf"), bbox_inches = 'tight', format = 'pdf')

    # plot session ap stats histograms (per mac addr.)
    fig_2 = plt.figure(figsize = (12, 12))

    ax1 = fig_2.add_subplot(221)
    ax1.set_title('median contact time per AP, \nover all sessions')

    ax1.xaxis.grid(False)
    ax1.yaxis.grid(True)
    ax1.hist(session_stats['ap_stats']['coverage_time'], bins = np.arange(0, 80 + 5, 5), histtype = 'bar', rwidth = 0.8)
    ax1.set_xlabel("contact time (s)")
    ax1.set_ylabel("nr. of AP contacts")
    # ax1.set_xticks(np.arange(0, 80 + 5, step = 5))

    ax2 = fig_2.add_subplot(222)
    ax2.set_title('median contact range per AP, \nover all sessions')

    ax2.xaxis.grid(False)
    ax2.yaxis.grid(True)
    ax2.hist(session_stats['ap_stats']['coverage_dist'], bins = np.arange(0, 80 + 5, 5), histtype = 'bar', rwidth = 0.8)
    ax2.set_xlabel("contact range (m)")
    ax2.set_ylabel("nr. of AP contacts")
    # ax2.set_xticks(np.arange(0, 80 + 5, step = 5))

    ax3 = fig_2.add_subplot(223)
    ax3.set_title('median contact speed per AP, \nover all sessions')

    ax3.xaxis.grid(False)
    ax3.yaxis.grid(True)
    ax3.hist(session_stats['ap_stats']['coverage_speed'], bins = np.arange(0, 50 + 5, 5), histtype = 'bar', rwidth = 0.8)
    ax3.set_xlabel("contact speed (m/s)")
    ax3.set_ylabel("nr. of AP contacts")
    ax3.set_xticks(np.arange(0, 50 + 5, step = 5))

    # get unique mac addresses of aps
    ax4 = fig_2.add_subplot(224)
    ax4.set_title('# of APs w/ authentication\nmethod x')

    ax4.xaxis.grid(False)
    ax4.yaxis.grid(True)
    ax4.hist(session_stats['ap_locations']['auth'], bins = np.arange(0, 6 + 1, 1), histtype = 'bar', rwidth = 0.8)
